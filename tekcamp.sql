...
SELECT *
FROM tekcampers
WHERE education IN ("High School", "Some College");

-- Create table for ta-mark
CREATE TABLE `ta-mark`
(
    student_id INT,
    FOREIGN KEY
(student_id) REFERENCES tekcampers
(id)
)
...
(35),
(37);
-- Showing the students in ta-marks full information
SELECT tekcampers.*
FROM tekcampers
    INNER JOIN `ta  -mark
`
...
-- Adding hobby id to tekcampers table
...
-- Showing the students in ta-marks full information with hobbies *NOTE: Only returns people with set hobbies values*
